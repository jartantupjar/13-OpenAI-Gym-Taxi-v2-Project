...
class Agent:
    ...
    def __init__(self, nA=6):
        ...
        self.Epsilon=.64
        self.Counter=0
        self.Gamma=.65 # .8 is decent .6 better
        self.Alpha=.6
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if not done:
            ...
            self.Counter += 1
           # self.Epsilon = 1 / np.exp(self.Alpha * self.Counter)
           ...
           # Qsa_next = self.Q[next_state][next_action]
            ...
            self.Q[state][action] = self.Q[state][action] + (alpha*(reward+(gamma*Qsa_next)-self.Q[state][action]))